...
package io.github.autotweaker.adapter.cli.commands.status
...
class Status : Command {
	...
	override fun handle(
		...
		emit(Command.Chunk.Data(keystoreState))
		
		val adapters = core.listAdapter()
		emit(Command.Chunk.Data(""))
		emit(Command.Chunk.Data(I18n.get("status.adapters", adapters.size)))
		for (a in adapters) {
			emit(Command.Chunk.Data("  ${a.name}  ${a.version}  ${a.description}"))
		}
		
		val providers = core.config.listProviders()
		emit(Command.Chunk.Data(""))
		emit(Command.Chunk.Data(I18n.get("status.providers", providers.size)))
		for (p in providers) {
			emit(Command.Chunk.Data("  ${p.displayName}  ${p.type}"))
		}
		
		val models = core.config.listModels()
		emit(Command.Chunk.Data(""))
		emit(Command.Chunk.Data(I18n.get("status.models", models.size)))
		for (m in models) {
			emit(Command.Chunk.Data("  ${m.data.displayName}"))
		}
		...
		emit(Command.Chunk.Data(""))
		emit(Command.Chunk.Data(I18n.get("status.sessions", sessions.size)))
		for (s in sessions) {
			val handle = core.session.getHandle(s.id)
			val state = handle?.status?.value ?: I18n.get("status.inactive")
			emit(Command.Chunk.Data("  ${s.id}  $state  ${s.title}"))
		}
		
		emit(Command.Chunk.Data(""))
		emit(Command.Chunk.Data(I18n.get("status.workspaces", workspaces.size)))
		for (w in workspaces) {
			emit(Command.Chunk.Data("  ${w.meta.name}  ${w.meta.path}"))
		}
