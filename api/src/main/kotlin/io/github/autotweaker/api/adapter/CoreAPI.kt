 ...
 *
 * 基础设施类的 API 通过 able 接口暴露，与 [CoreAPI] 无重叠。
 *
...
interface CoreAPI {
	...
	interface SessionAPI {
		 ...
		 * 获取历史的全部 Usage
		 *
		 * @return key 为消息 id，value 为 [UsageSnapshot]
		 * @see UsageSnapshot
		 */
		 ...
		 * 创建新的工作区
		 *
		 * @throws IllegalArgumentException displayName 与已有工作区重复
		 * @throws IllegalStateException path 不是一个目录
		 * @return 新工作区的 data
		 */
		...
		/**
		 * 容器的启停由 AutoTweaker 内部管理，按需自动启动，不应该将此值作为检查项
		 *
	...
	interface ConfigAPI {
		...
		/**
		 * @throws IllegalArgumentException 存在同名 key
		 */
		suspend fun addApiKey(key: CoreConfig.ProviderConfig.ApiKey)
		
		/**
		 * @throws IllegalStateException key 正在使用
	...
	interface SecretAPI {
		 ...
		 *
		 * @return 密钥库未解锁也返回 false，不代表密码已设置，请同时检查 [isUnlocked]
		 */
	...
	/**
	 * 管理 i18n 自动翻译的 API，AutoTweaker 在后台对已注册的 i18n 条目进行自动翻译。
	 *
	 * 如果翻译模型未设置，翻译将不会进行。
	 *
	...
	/**
	 * trace 的管理 API，支持读取和删除。
	 *
	 * @see io.github.autotweaker.api.trace.TraceRecorder
	 */
	interface TraceAPI {
		/**
		 * 列出所有记录者的 id，id 是 `KClass.java.name`。
		 */
		 ...
		 *
		 * @return 是否成功删除了大于或等于 1 条数据，通常也只会删除一条。
		 */
