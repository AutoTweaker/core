...
object WriteDesc {
	...
	class Tool : StringSetting(
		"创建一个新文件，或覆写一个已有文件，支持Unicode转义。优先使用edit来更新文件的部分片段。\n在任何情况下不要使用bash来创建文件，而是激活此工具来写入文件，即使要写入特殊字符。",
		zh("write工具的未激活描述")
	)
	
	@AutoService(SettingDef::class)
	class Function : StringSetting(
		"创建一个新文件，或覆写已有文件。对于覆写，你需要确保提前通过read工具读取目标文件。\n" +
				"支持unicode转义，不支持\\n等json转义，需要通过unescape_unicode显式启用。\n" +
				"你应该优先使用edit来更新文件的部分片段。在任何情况下你都应该始终避免使用bash来创建文件。\n" +
	...
	class UnescapeUnicode : StringSetting(
		"是否对content中的Unicode转义序列进行解码，默认false。仅支持Unicode转义以及反斜杠转义，例如content中的字面'\\u0055'将被解析为'U'，'\\\\u0055'将被解析为'\\u0055'，不支持JSON转义如'\\n'或'\\t'",
