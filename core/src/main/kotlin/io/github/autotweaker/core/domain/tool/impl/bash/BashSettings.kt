...
object BashSettings {
	...
	class Description : StringSetting(
		"运行一条bash命令，可选设置超时时间并按id注入一次性环境变量", "bash工具的描述"
	)
	
	@AutoService(SettingDef::class)
	class RunFuncDescription : PromptSetting(
		"bash_run", "bash_run工具的描述"
	)
	
	@AutoService(SettingDef::class)
	class CommandPropDescription : StringSetting(
		"要执行的bash命令内容", "bash_run工具command参数的描述"
	)
	
	@AutoService(SettingDef::class)
	class TimeoutPropDescription : StringSetting(
		"命令超时时间（秒），必须大于0，默认%s秒", "bash_run工具timeout_seconds参数的描述"
	)
	...
	class InvalidTimeoutMessage : StringSetting(
		"timeout_seconds必须大于0", "bash_run工具timeout_seconds非法时的描述"
	)
	
	@AutoService(SettingDef::class)
	class InvalidCommandMessage : StringSetting(
		"command参数不能为空", "bash_run工具command非法时的描述"
	)
