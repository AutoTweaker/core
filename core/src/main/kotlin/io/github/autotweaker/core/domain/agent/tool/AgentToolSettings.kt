...
object AgentToolSettings {
	...
	class Rejected : StringSetting(
		"工具调用已被用户拒绝，$TOOL_NOT_EXECUTED。请停止当前操作；" +
				"向用户解释为什么要执行这个操作；询问用户意见；等待用户告知如何继续", zh(
			"工具调用被拒绝时的ToolResult"
		)
	)
	...
	class RejectedWithFeedback : StringSetting(
		"$TOOL_NOT_EXECUTED，用户拒绝了工具调用，并留言：%s", zh(
			"工具调用被拒绝，有原因时的ToolResult"
		)
	)
	...
	class TimeoutSeconds : IntSetting(
		1800, zh(
			"工具调用超时时间，单位秒，超时后工具将中止并丢弃响应，谨慎设置"
		)
	)
	
	@AutoService(SettingDef::class)
	class TimeoutMessage : StringSetting(
		"工具调用超时（%s秒），工具可能已经对文件系统或外部环境产生影响，在继续之前请完成确认", zh(
			"工具调用超时后的ToolResult"
		)
	)
	...
	class ActiveMessage : StringSetting(
		"工具已激活，包含这些function：[%s]\n" +
				"注意：名为[%s]的function已不再可用，检查你的工具列表来了解新的function和使用方法", zh(
			"激活工具后的ToolResult"
		)
	)
