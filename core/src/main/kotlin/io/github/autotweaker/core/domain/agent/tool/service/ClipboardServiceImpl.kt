...
class ClipboardServiceImpl(
	...
	override suspend fun read(clipId: UUID): String? =
		storage.read(clipId, inContainer)
	
	override suspend fun path(clipId: UUID): Path? {
		val hostPath = storage.list(inContainer)[clipId] ?: return null
		return if (inContainer) pathResolver.toContainerPath(hostPath) else hostPath
	}
}
