...
val defaultItems: List<SettingItem> = listOf(
	...
	// region Agent — 工具调用超时
	SettingItem(
		SettingKey("core.agent.tool.timeout.seconds"), SettingValue.ValInt(600), "工具调用超时时间，单位秒"
	),
	SettingItem(
		SettingKey("core.agent.tool.response.timeout"),
		SettingValue.ValString("工具调用超时（%s秒）"),
		"工具调用超时后的ToolResult"
	),
	// endregion
	...
	SettingItem(
		...
		SettingValue.ValString("激活此工具以开始使用，无论将此值设为true或false都将启用工具"),
		"未激活工具的enable属性描述"
	),
	SettingItem(
		SettingKey("core.agent.tool.response.active"),
		SettingValue.ValString("工具%s已激活，包含%s个function，检查你的工具列表来了解如何使用"),
		"激活工具后的ToolResult"
	),
	// endregion
	...
	SettingItem(
		...
		SettingValue.ValString("提供的路径不合法，请检查提供的路径参数"),
		"路径解析失败时的描述"
	),
	// endregion
	...
	SettingItem(
		...
		SettingValue.ValString("你是文件总结助手，请根据用户输入和以下指令生成关于文件内容的摘要"),
	...
	SettingItem(
		...
		SettingValue.ValString("读取一个文件，支持获取摘要以及Unicode代码"),
	...
	SettingItem(
		...
		SettingValue.ValString("用于总结文件的提示词，调整此字段来要求总结器关注不同细节"),
		"read_summarize工具prompt参数的描述"
	),
	SettingItem(
		SettingKey("core.tool.read.function.description.unicode.property.max.chars"),
		SettingValue.ValString("读取文件的前n个字符，这将包括换行符等特殊字符，最多%s个字符"),
	...
	SettingItem(
		...
		SettingValue.ValString("<字符数过多，后续内容已被截断（共%s字符）>"),
	...
	SettingItem(
		...
		SettingValue.ValString("运行一条bash命令，可选设置超时时间并按id注入一次性环境变量"),
		"bash工具的描述"
	),
	...
	SettingItem(
		...
		SettingValue.ValString("要执行的bash命令内容"),
		"bash_run工具command参数的描述"
	),
	SettingItem(
		SettingKey("core.tool.bash.property.description.timeout.seconds"),
		SettingValue.ValString("命令超时时间（秒），必须大于0，默认%s秒"),
		"bash_run工具timeout_seconds参数的描述"
	),
	SettingItem(
		SettingKey("core.tool.bash.property.description.env.ids"), SettingValue.ValString(
	...
	SettingItem(
		...
		SettingValue.ValString("timeout_seconds必须大于0"),
		"bash_run工具timeout_seconds非法时的描述"
	),
	SettingItem(
		SettingKey("core.tool.bash.message.error.invalid.command"),
		SettingValue.ValString("command参数不能为空"),
		"bash_run工具command非法时的描述"
	),
	SettingItem(
		SettingKey("core.tool.bash.message.result.template"), SettingValue.ValString(
			"""
			命令已执行，退出码：%s，执行时间：%s秒
			标准输出：
			<stdout>
	...
	SettingItem(
		...
		SettingValue.ValInt(10000),
		"上下文压缩前对字符数大于此值的消息进行单独总结"
	),
	SettingItem(
		SettingKey("core.agent.compact.message.summarize.prompt"), SettingValue.ValString(
			"请对以下消息内容进行概括，输出不要太长\n\n" + "<message>\n%s\n</message>"
	...
	SettingItem(
		...
		SettingValue.ValString("buildpack-deps:stable"),
